...
const Login: NextPage = ({ csrfToken }: any) => {
    ...
    useEffect(() => {
        if (router.query && router.query?.callbackUrl) {
            router.prefetch(`${router.query.callbackUrl}`);
        } else {
            router.prefetch('/')
        }
    }, [router])

    const onSubmit = (account: FormValues) => {
        setButtonDisable(true);
        signIn("credentials", { ...account, redirect: false })
            .then(({ ok, error, status, url }: any) => {
                if (ok) {
                    setButtonDisable(false);
                    setErrorMessage('');
                    router.push(router.query?.callbackUrl?.toString() ? router.query?.callbackUrl.toString() : '/');
                } else {
                    setButtonDisable(false);
                    setErrorMessage('Số điện thoại hoặc Mật khẩu không đúng, vui lòng thử lại!');
                }
            })
            .catch((err) => {
                setButtonDisable(false);
            ...
            <Header title="Đăng Nhập" />
            {
                loading ? <Loading2 /> : (
                    <div className="min-h-screen flex justify-center items-center">
                        ...
                        <div className="w-screen h-screen flex flex-col justify-center items-center lg:grid lg:grid-cols-2 mt-0">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            ...
                            <div className="flex flex-col justify-center items-center w-1/2">
                                ...
                                <h2 className="my-4 font-display font-bold text-3xl text-gray-700 text-center">
                                    Đăng nhập
                                </h2>
                                <div className="flex-auto w-[320px] sm:w-[320px] md:w-[360px] px-2 lg:px-0 py-3 bg-transparent">
                                    <form onSubmit={handleSubmit(onSubmit)}>
                                        ...
                                        <div className="text-center">
                                            <button type="submit"
                                                className={classNames("bg-red-600", "px-4 py-2 rounded-md text-base w-full text-white")}>
                                                {
                                                    buttonDisable ? (
                                                        <>
                                ...
                                <div className="w-[320px] sm:w-[320px] md:w-[360px] px-2 lg:px-0 flex items-center mt-2 mb-4 before:flex-1 before:border-t before:border-gray-300 before:mt-0.5 after:flex-1 after:border-t after:border-gray-300 after:mt-0.5">
                                    <p className="text-center font-normal text-sm text-gray-400 mx-4 mb-0">HOẶC</p>
                                </div>
                                <div className="flex items-center flex-col sm:flex-col md:flex-row w-[320px] sm:w-[320px] md:w-[360px] px-2 lg:px-0">
                                    {/* <form method="post" action="/api/auth/signin/google" className="w-1/2 mr-2"> */}
                                    {/* <input name="csrfToken" type="hidden" defaultValue={csrfToken} /> */}
                                    ...
                                    {/* </form> */}
                                    {/* <form method="post" action="/api/auth/signin/facebook" className="w-1/2 ml-2"> */}
                                    {/* <input name="csrfToken" type="hidden" defaultValue={csrfToken} /> */}
                                ...
                                <div className="px-6 pt-2 pb-6 mt-4 w-screen sm:w-screen md:w-full">
                                    <div className="text-center text-gray-600">Bạn chưa có tài khoản? <Link href={"/dangky"}><a className="font-medium text-red-500 hover:underline">Đăng ký ngay</a></Link></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
export const getServerSideProps = async (context: any) => {
    ...
    return {
        props: { session, csrfToken: csrfToken || null, }
    }
}
